...
output:
  pdf_document:
    extra_dependencies: ["setspace", "float"]
---
...
for investigation of drug mechanisms *in vivo* (**REFS**).
...
lost with cell lines (REF Tentler, Pompili)
...
to clinical stages (REF PDXnet Website). Much of the work from the PDXNet
has focused on genomic characterization and studies identifying
vulnerabilities and resistances to treatment (REFS).
While these studies often assess tumor growth over time,
the methods used to test differences are often limited
to t-tests and ANOVA (REFS). Evrard et al surveyed a variety of methods
...
baseline and the final measurement (REF Evrard, NCI?).
...
recommend harvesting tumors at 1000-2000mg (REF PDMR SOP).
...
All data processing and analysis were conducted using R/v4.1.0 (REF R core).
...
Models were fit using the R package `lme4`/v1.1.27.
...
with the R package `survival`/v3.2.13 (REF R package)
...
which is in line with recommendations from the PDMR on harvesting volumes (REF NCI).
...
Therefore the statistic of interest is the slope,
which in the mixed model represents the change in the ratio of the
mean tumor volume in treatment versus control per month.
...
were 0.60 (95% CI: 0.43, 0.85) and 0.71 (95% Wald CI: 0.51, 0.99) which correspond to a
40% and 29% reduction in the growth rate relative to control, respectively (Fig. 3).
...
\begin{center}
\begin{figure}[H]
\includegraphics[width = 6in, height = 5in]{figures/aft_models_overlaid_forest.pdf}
...
Each have been shown to improve overall survival (REF labianca, fujita, nci)
...
with a *BRAF*^V600E^ mutation (REF kopetz), but is not FDA approved for
...
rather than simply being missing. (REF Tan)
...
if "clinical signs are noted" (REF PDMR SOP). Therefore,
...
1) Tentler JJ, Tan AC, Weekes CD, et al. Patient-derived tumour xenografts as models for oncology drug development. Nat Rev Clin Oncol. 2012;9(6):338-350. Published 2012 Apr 17. doi:10.1038/nrclinonc.2012.61
The NCI Patient-Derived Models Repository (PDMR), NCI-Frederick, Frederick National Laboratory for Cancer Research, Frederick, MD. https://pdmr.cancer.gov/. 
Drugs approved for colon and rectal cancer. National Cancer Institute. https://www.cancer.gov/about-cancer/treatment/drugs/colorectal#1. Accessed April 30, 2022. 
Tan M, Fang HB, Tian GL, Houghton PJ. Repeated-measures models with constrained parameters for incomplete data in tumour xenograft experiments. Stat Med. 2005;24(1):109-119. doi:10.1002/sim.1775
Fujita K, Kubota Y, Ishida H, Sasaki Y. Irinotecan, a key chemotherapeutic drug for metastatic colorectal cancer. World J Gastroenterol. 2015;21(43):12234-12248. doi:10.3748/wjg.v21.i43.12234
Labianca R, Beretta GD, Kildani B, et al. Colon cancer. Crit Rev Oncol Hematol. 2010;74(2):106-133. doi:10.1016/j.critrevonc.2010.01.010
